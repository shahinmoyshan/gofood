 ...
 */
class GoFood_Bread extends WP_List_Table {
	...
	/**
	 * Constructor for the GoFood_Bread class.
	 *
	...
	public function __construct( $setup ) {
		...
		if ( ! isset( $setup['bread']['edit']['fields'] ) ) {
	...
	/**
	 * Setup a new instance of GoFood_Bread.
	 *
	 * @param array $setup The configuration array for setting up the table.
	 * @return self
	 */
	 ...
	 */
	protected function apply_filters( $key, $data ) {
		if ( isset( $this->bread_filters[ $key ] ) && is_callable( $this->bread_filters[ $key ] ) ) {
			$data = call_user_func( $this->bread_filters[ $key ], $data );
		}
	...
	public function wp_list_table_data( $order_by = '', $order = '', $search_term = '' ) {
		...
		if ( ! isset( $this->table_setup['total_record'] ) ) {
			$this->table_setup['total_record'] = GoFood_Query::total( $this->table_setup['table'], $args );
		}
		...
		return GoFood_Query::results( $this->table_setup['table'], $args );
	}
	...
	public function get_bread_view( $view ) {
		...
		$root_path = $this->table_setup['bread']['template'] ?? rtrim( gf_partials_path( 'bread/', false, true ), '.php' );
	...
	public function render() {
		...
		if ( isset( $_GET['action'] ) && in_array( $_GET['action'], array( 'new', 'edit' ) ) ) {
			...
			if ( isset( $_SERVER['REQUEST_METHOD'] ) && 'POST' === $_SERVER['REQUEST_METHOD'] ) {
				...
				// nonce validation.
				if ( ! isset( $_POST['gofood_bread_nonce'] ) || ! wp_verify_nonce( $_POST['gofood_bread_nonce'], 'gofood_bread' ) ) {
				...
				if ( $error_message === null ) {
					if ( isset( $_POST['update_id'] ) ) {
						...
						if ( GoFood_Query::update( $table, $data, array( $this->get_primary_key() => $_POST['update_id'] ) ) ) {
						...
						$created_id = GoFood_Query::create( $table, $data );
			...
			if ( isset( $_GET['action'] ) && 'edit' === $_GET['action'] && isset( $_GET[ $this->get_primary_key() ] ) ) {
				...
				$edit_data = GoFood_Query::result( $table, array( 'where' => $this->get_primary_key() . ' = ' . $_GET[ $this->get_primary_key() ] ) );
			...
			if ( isset( $_GET['action'] ) && 'delete' === $_GET['action'] && isset( $_GET[ $this->get_primary_key() ] ) ) {
				...
				if ( GoFood_Query::query( 'DELETE FROM ' . GoFood_Query::table( $table ) . " WHERE {$this->get_primary_key()} IN(" . join( ',', $ids ) . ')' ) ) {
			...
			if ( isset( $_GET['action'] ) && 'view' === $_GET['action'] && isset( $_GET[ $this->get_primary_key() ] ) ) {
				$record = GoFood_Query::result( $table, array( 'where' => $this->get_primary_key() . ' = ' . $_GET[ $this->get_primary_key() ] ) );
